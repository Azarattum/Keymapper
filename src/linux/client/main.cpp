...
int main(int argc, char* argv[]) {
  ...
  for (;;) {
    ...
    verbose("connecting to hotkeyerd");
    ...
    // send configuration
    verbose("sending configuration to hotkeyerd");
    if (send_config(ipc_fd, config_file.config())) {
      ...
      for (;;) {
        ...
        if (is_pipe_broken((ipc_fd))) {
          verbose("connection to hotkeyerd lost");
          break;
        }
        ...
        if (focused_window && update_focused_window(*focused_window)) {
          ...
          if (active_override_set != override_set) {
            if (override_set >= 0) {
              verbose("sending 'active context #%i' to hotkeyerd:", override_set + 1);
              ...
              verbose("sending 'no active context' to hotkeyerd");
            }
